...
public:
	void OnTargetHit__DelegateSignature(class AActor* HitActor, const struct FVector& HitLocation, const struct FVector& HitNormal, EPhysicalSurface SurfaceType, bool bDisableDamage, bool bIsPenetration_0);
	void OnBlocking__DelegateSignature(class AActor* BlockedActor_0, const struct FVector& HitLocation, float CuttingRate, bool bInvincible, bool bIsPenetration_0);
	void OnTargetOverlap__DelegateSignature(class AActor* Target, class AActor* DamageCauser, const struct FVector& HitLocation);
	void OnPenetrateHit__DelegateSignature(const struct FVector& Location, const struct FVector& Normal, bool bCharacter);
...
static_assert(sizeof(ABP_AJBDamageActor_C) == 0x000478, "Wrong size on ABP_AJBDamageActor_C");
